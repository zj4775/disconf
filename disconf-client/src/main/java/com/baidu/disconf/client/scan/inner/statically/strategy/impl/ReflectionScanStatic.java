...
import com.baidu.disconf.client.common.annotations.*;
...
public class ReflectionScanStatic implements ScanStaticStrategy {
    ...
    private void analysis4DisconfFile(ScanStaticModel scanModel) {
        ...
        while (iterator.hasNext()) {
            ...
            // 校验配置文件类型是否合适(目前只支持.properties,yml类型)
    ...
    private ScanStaticModel scanBasicInfo(List<String> packNameList) {
        ...
        //
        // 获取DisconfYmlFile class
        //
        Set<Class<?>> classdata = reflections.getTypesAnnotatedWith(DisconfYmlFile.class);
        scanModel.setDisconfYmlFileClassSet(classdata);

        //
        // 获取DisconfFile class
        //
        classdata = reflections.getTypesAnnotatedWith(DisconfFile.class);
