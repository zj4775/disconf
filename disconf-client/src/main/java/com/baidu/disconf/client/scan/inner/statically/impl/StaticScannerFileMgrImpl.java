...
public class StaticScannerFileMgrImpl extends StaticScannerMgrImplBase implements StaticScannerMgr {
    ...
    public void scanData2Store(ScanStaticModel scanModel) {
        ...
        // 转换yml配置文件
        List<DisconfCenterBaseModel> disconfYmlCenterFiles = getDisconfYmlFiles(scanModel);
        DisconfStoreProcessorFactory.getDisconfYmlStoreFileProcessor().transformScanData(disconfYmlCenterFiles);
    }
     ...
     * 获取配置文件数据
     */
    private static List<DisconfCenterBaseModel> getDisconfYmlFiles(ScanStaticModel scanModel) {

        List<DisconfCenterBaseModel> disconfCenterFiles = new ArrayList<DisconfCenterBaseModel>();
        /**
         * 处理DisconfYmlFile注解
         */
        Set<Class<?>> classSet = scanModel.getDisconfYmlFileClassSet();
        for (Class<?> disconfYmlFile : classSet) {

            DisconfYmlCenterFile disconfCenterFile = transformScanYmlFile(disconfYmlFile);
     ...
     * 转换yml配置文件
     */
    private static DisconfYmlCenterFile transformScanYmlFile(Class<?> disconfYmlFileClass) {

        DisconfYmlCenterFile disconfYmlCenterFile = new DisconfYmlCenterFile();

        //
        // class
        disconfYmlCenterFile.setCls(disconfYmlFileClass);

        DisconfYmlFile disconfYmlFileAnnotation = disconfYmlFileClass.getAnnotation(DisconfYmlFile.class);

        //
        // file name
        disconfYmlCenterFile.setFileName(disconfYmlFileAnnotation.filename());

        // config file target dir path
        disconfYmlCenterFile.setTargetDirPath(disconfYmlFileAnnotation.targetDirPath().trim());

        // file type
        disconfYmlCenterFile.setSupportFileTypeEnum(SupportFileTypeEnum.getByFileName(disconfYmlFileAnnotation.filename()));

        //
        // disConfCommonModel
        DisConfCommonModel disConfCommonModel =
                makeDisConfCommonModel(disconfYmlFileAnnotation.app(), disconfYmlFileAnnotation.env(), disconfYmlFileAnnotation
                        .version());
        disconfYmlCenterFile.setDisConfCommonModel(disConfCommonModel);
        ...
        String url = DisconfWebPathMgr.getRemoteUrlParameter(DisClientSysConfig.getInstance().CONF_SERVER_STORE_ACTION,
                ...
                disConfCommonModel.getVersion(),
                disConfCommonModel.getEnv(),
                disconfYmlCenterFile.getFileName(),
                DisConfigTypeEnum.FILE);
        disconfYmlCenterFile.setRemoteServerUrl(url);

        return disconfYmlCenterFile;
    }
