...
public class ConfigMgrImpl implements ConfigMgr {
    ...
    public List<File> getDisconfFileList(ConfListForm confListForm) {
        ...
        for(Config c:configList) {
        	c.setValue(CodeUtils.unicodeToUtf8(c.getValue()));
        }
     ...
     *
     */
    private List<String> compareConfig(String zkData, String dbData,String fileType) {

        List<String> errorKeyList = new ArrayList<String>();
        Properties prop = new Properties();
        if(YML.equals(fileType)){
            YamlMapFactoryBean yamlMapFactoryBean=new YamlMapFactoryBean();
            yamlMapFactoryBean.setResources(new ByteArrayResource(dbData.getBytes()));
            Map<String, Object> map = YamlFileUtils.readYamlMap(yamlMapFactoryBean.getObject());
            for (Map.Entry<String, Object> entry : map.entrySet()) {
                prop.put(entry.getKey(),entry.getValue());
            }
        }else if(PROPERTIES.equals(fileType)){
            try {
                BufferedReader bf = new BufferedReader(new InputStreamReader(IOUtils.toInputStream(dbData), "UTF-8"));
                prop.load(bf);
            } catch (Exception e) {
    ...
    private MachineListVo getZkData(List<ZkDisconfDataItem> datalist, Config config) {
        ...
        for (ZkDisconfDataItem zkDisconfDataItem : datalist) {
            ...
            if (config.getType().equals(DisConfigTypeEnum.FILE.getType())) {
                String fileType=config.getName().substring(config.getName().lastIndexOf(".")+1);
                List<String> errorKeyList = compareConfig(zkDisconfDataItem.getValue(), config.getValue(),fileType);
